<?php 
		...
		if(isset($_POST['create'])){
			...
			{  $insert=array('name'=>$name,'priority'=>$priority,'date'=>date('Y-m-d'),'status'=>1);
		...
		{
			extract($_POST);if(!empty($name) && !empty($priority))
			{$updatearray=array("id"=>$id);$upd2=array('name'=>$name,'priority'=>$priority,'date'=>date('Y-m-d'),'status'=>1);
